...
namespace mdt112_assignment2
{
    ...
    class Quiz
    {
        ...
        public static List<Garbage> SortGarbageListBySize(List<Garbage> garbageList, bool isReverse)
        {
            ...
            for (int a = 0; a < garbageList.Count; a++)
            {
                Size x = garbageList[a].GetSize();

                if (x == Size.SMALL)
                {
                    outputgarbageList.Add(garbageList[a]);
                }
            }
            for (int b = 0; b < garbageList.Count; b++)
            {
                Size x = garbageList[b].GetSize();

                if (x == Size.MEDIUM)
                {
                    outputgarbageList.Add(garbageList[b]);
                }
            }
            for (int c = 0; c < garbageList.Count; c++)
            {
                Size x = garbageList[c].GetSize();

                if (x == Size.LARGE)
                {
                    outputgarbageList.Add(garbageList[c]);
                }
            }
